...
const MyAccount = (props) => {
    ...
    useEffect(() => {
        const userId = sessionStorage.getItem('user_id'); // Get the user ID from the session storage or any other appropriate storage
      
        // Make a fetch request to the server API to get all users
        fetch('http://127.0.0.1:4000/user', {
          ...
          .then((data) => {
            const user = data.find((user) => user.id === userId); // Find the user based on the ID
            if (user) {
              setEmail(user.email); // Set the email of the found user
            }
          })
          .catch((error) => {
            console.error(error);
                        ...
                        <Tab.Content>
                            <Tab.Pane eventKey="my-orders">
                                ...
                                <OrderCard 
                                    ...
                                    deliveredDate="20 may, 2023"
                                />
                                <OrderCard 
                                    orderDate="12 Jun, 2022" 
                                    orderId="1334" 
                                    title="Mens Casual Premium Slim Fit T-Shirts"
                                    img="https://fakestoreapi.com/img/71-3HjGNDUL._AC_SY879._SX._UX._SY._UY_.jpg"
                                    deliveredDate="22 Jun, 2022"
                                />
                            </Tab.Pane>
